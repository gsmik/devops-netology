...
[](/7.2/src/vms_platform.tf)


2. Так же поступите с блоком **metadata {serial-port-enable, ssh-keys}**, эта переменная должна быть общая для всех ваших ВМ.
3. Найдите и удалите все более не используемые переменные проекта.(закоментировал)
4. Проверьте terraform plan (изменений быть не должно). -небыло
